...
interface InputPanelProps {
  ...
  onCsvImport?: (locations: string[], importedRemarks: string[]) => void;
  onOptimize: () => void;
  onClear: () => void;
  lockedSlots: boolean[];
...
export default function InputPanel({
  ...
  const handleUploadCsv = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      skipEmptyLines: true,
      complete: ({ data }) => {
        const rows = (data as string[][]).filter((r) => r[0] && r[0].toLowerCase() !== "location");
        const locs = rows.map((r) => r[0]);
        const parsedRemarks = rows.map((r) => r[1] || "");
        onCsvImport(locs, parsedRemarks);
      },
    });
    e.target.value = "";
  };
