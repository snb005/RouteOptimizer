...
export default function RouteLayer({ locations, lockedSlots, onResult, onMapClick }: Props) {
  ...
  useEffect(() => {
    if (!map || locations.length < 2 || !geometryLib || !markerLib) return;
    ...
    const fetchRoute = async () => {
      ...
      orderedLocs.forEach((loc, idx) => {
        ...
        let markerContent = undefined;
        if (markerLib) {
          const pin = new markerLib.PinElement({
             background: hasRemark ? "#F59E0B" : ROUTE_COLORS[idx % ROUTE_COLORS.length],
             glyphColor: "#fff",
             borderColor: "#fff",
             glyph: String(idx + 1),
             scale: 1.2
  ...
  }, [map, locations, geometryLib, markerLib, lockedSlots]);
