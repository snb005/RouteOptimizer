...
export default function MapComponent({ apiKey }: { apiKey: string }) {
  ...
  const handleCsvImport = useCallback((locs: string[], importedRemarks?: string[]) => {
    ...
    setRemarks(importedRemarks && importedRemarks.length === locs.length ? importedRemarks : Array(locs.length).fill(""));
  ...
  const orderedLocations = (() => {
    ...
    if (optimizedOrder.length > 0 && intermediates.length > 0) {
      const mapped = optimizedOrder.map((i) => intermediates[i]).filter(Boolean);
      if (mapped.length === intermediates.length) {
        return [origin, ...mapped, destination];
      }
    }
    return locations;
  })();
